 ...
 * Theme activation
 *
 * Ralph: Wondering whether we still need this? Thoughts?
 *
 */
...
function roots_theme_activation_options_add_page() {
	...
	if ( ! $roots_activation_options ) {
		$theme_page = add_theme_page(
			__( 'Theme Activation', '@textdomain' ),
			__( 'Theme Activation', '@textdomain' ),
			'edit_theme_options',
...
function roots_theme_activation_options_render_page() { ?>
  <div class="wrap">
    <h2><?php printf(__('%s Theme Activation', '@textdomain'), wp_get_theme()); ?></h2>
    <div class="update-nag">
        <p><?php _e('These settings are optional and should usually be used only on a fresh installation', '@textdomain'); ?></p>
        <p><strong><?php _e('After theme activation, find and replace ",\'tend\' " to set the textdomain and replace \'tend\' in style.css', '@textdomain'); ?></strong></p>
    </div>
    <?php settings_errors(); ?>
    ...
    <form method="post" action="options.php">
      ...
      <table class="form-table">
        <tr valign="top"><th scope="row"><?php _e('Create static front page?', '@textdomain'); ?></th>
          <td>
            <fieldset>
              <legend class="screen-reader-text"><span><?php _e('Create static front page?', '@textdomain'); ?></span></legend>
              <select name="roots_theme_activation_options[create_front_page]" id="create_front_page">
                <option selected="selected" value="true"><?php echo _e('Yes', '@textdomain'); ?></option>
                <option value="false"><?php echo _e('No', '@textdomain'); ?></option>
              </select>
              <p class="description"><?php printf(__('Create a page called Home and set it to be the static front page', '@textdomain')); ?></p>
            </fieldset>
          </td>
        </tr>
        <tr valign="top"><th scope="row"><?php _e('Change permalink structure?', '@textdomain'); ?></th>
          <td>
            <fieldset>
              <legend class="screen-reader-text"><span><?php _e('Update permalink structure?', '@textdomain'); ?></span></legend>
              <select name="roots_theme_activation_options[change_permalink_structure]" id="change_permalink_structure">
                <option selected="selected" value="true"><?php echo _e('Yes', '@textdomain'); ?></option>
                <option value="false"><?php echo _e('No', '@textdomain'); ?></option>
              </select>
              <p class="description"><?php printf(__('Change permalink structure to /&#37;postname&#37;/', '@textdomain')); ?></p>
            </fieldset>
          </td>
        </tr>
        <tr valign="top"><th scope="row"><?php _e('Create navigation menu?', '@textdomain'); ?></th>
          <td>
            <fieldset>
              <legend class="screen-reader-text"><span><?php _e('Create navigation menu?', '@textdomain'); ?></span></legend>
              <select name="roots_theme_activation_options[create_navigation_menus]" id="create_navigation_menus">
                <option selected="selected" value="true"><?php echo _e('Yes', '@textdomain'); ?></option>
                <option value="false"><?php echo _e('No', '@textdomain'); ?></option>
              </select>
              <p class="description"><?php printf(__('Create the Primary Navigation menu and set the location', '@textdomain')); ?></p>
            </fieldset>
          </td>
        </tr>
        <tr valign="top"><th scope="row"><?php _e('Add pages to menu?', '@textdomain'); ?></th>
          <td>
            <fieldset>
              <legend class="screen-reader-text"><span><?php _e('Add pages to menu?', '@textdomain'); ?></span></legend>
              <select name="roots_theme_activation_options[add_pages_to_primary_navigation]" id="add_pages_to_primary_navigation">
                <option selected="selected" value="true"><?php echo _e('Yes', '@textdomain'); ?></option>
                <option value="false"><?php echo _e('No', '@textdomain'); ?></option>
              </select>
              <p class="description"><?php printf(__('Add all current published pages to the Primary Navigation', '@textdomain')); ?></p>
            </fieldset>
          </td>
        </tr>
      </table>
...
function roots_theme_activation_action() {
  ...
  if ($roots_theme_activation_options['create_front_page'] === 'true') {
    ...
    $default_pages = array(__('Home', '@textdomain'));
  ...
  if ($roots_theme_activation_options['create_navigation_menus'] === 'true') {
    ...
    $primary_nav = wp_get_nav_menu_object(__('Primary Navigation', '@textdomain'));

    if (!$primary_nav) {
      $primary_nav_id = wp_create_nav_menu(__('Primary Navigation', '@textdomain'), array('slug' => 'primary_navigation'));
